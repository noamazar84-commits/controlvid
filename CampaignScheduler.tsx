...
// Initialize Firestore with long polling fallback to avoid gRPC ECONNRESET stream drops
...
// -----------------------------------------------------------------------------
// Firestore Error Context Handler
// -----------------------------------------------------------------------------
...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null): FirestoreErrorInfo {
  ...
  if (errMessage.includes("UNAVAILABLE") || errMessage.includes("ECONNRESET") || errMessage.includes("Code: 14")) {
    console.warn(`[Firestore Network Warning] Connection reset during ${operationType} on ${path}:`, errMessage);
  }
...
const LOCAL_USERS_KEY = "controlvid_fallback_users";
const LOCAL_FIN_KEY = "controlvid_fallback_financials";
const LOCAL_LOGS_KEY = "controlvid_fallback_logs";
...
// Plan & Credit Quota Mapper for Whop Payments
export function getQuotaAndTierForWhopPlan(planNameOrAmount?: string | number): { planName: string; credits: number } {
  if (typeof planNameOrAmount === "string" && planNameOrAmount.trim() !== "") {
    const p = planNameOrAmount.toLowerCase();
    if (p.includes("empire")) return { planName: "Empire", credits: 300 };
    if (p.includes("velocity")) return { planName: "Velocity", credits: 190 };
    if (p.includes("growth")) return { planName: "Growth", credits: 120 };
    if (p.includes("spark")) return { planName: "Spark", credits: 60 };
  }
  
  const amount = typeof planNameOrAmount === "number" ? planNameOrAmount : parseFloat(String(planNameOrAmount)) || 0;
  if (amount >= 180) return { planName: "Empire", credits: 300 };
  if (amount >= 105) return { planName: "Velocity", credits: 190 };
  if (amount >= 65) return { planName: "Growth", credits: 120 };
  if (amount >= 35) return { planName: "Spark", credits: 60 };
  return { planName: "Spark", credits: 60 };
}

// Autopilot Whop Webhook Handler for automated payment and subscription events
export async function processWhopWebhookPayment(
  email: string,
  amount: number,
  rawPlanName?: string,
  ...
  const normalizedEventType = (eventType || "payment.succeeded").toLowerCase();

  // Handle cancellation/invalidation events
  if (normalizedEventType.includes("invalid") || normalizedEventType.includes("cancel") || normalizedEventType.includes("deleted")) {
    if (user) {
      const userId = user.serialId;
      try {
        const usersCol = collection(db, "users");
        const snapshot = await getDocs(query(usersCol));
        const userDoc = snapshot.docs.find(d => d.data().email.toLowerCase() === email.toLowerCase());
        ...
        console.warn("[Firebase] LocalStorage fallback for Whop cancellation", err);
      }

      const localUsers = getLocalUsers();
      saveLocalUsers(localUsers.map(u => u.serialId === userId ? { ...u, subscription_status: "inactive" } : u));
      await createUsageLog(userId, email, "WHOP_SUBSCRIPTION_CANCELLED", 0.0);
    }
    return { success: true, status: "inactive", email };
  }

  // Determine plan and credit allocation
  const { planName, credits } = getQuotaAndTierForWhopPlan(rawPlanName || amount);

  let userId: number;
  const now = new Date().toISOString();
  const customerId = whopCustomerId || (user ? user.whop_customer_id : `whop_cust_${Math.random().toString(36).substring(2, 6)}`);

  if (!user) {
    user = await createUser(email, customerId, planName);
    userId = user.serialId;
  } else {
    userId = user.serialId;
  }

  const assignedCreditBalance = credits;
  ...
  try {
    ...
    if (userDoc) {
      await updateDoc(doc(db, "users", userDoc.id), {
        ...
        credit_balance: assignedCreditBalance,
        whop_customer_id: customerId,
        whop_membership_id: whopMembershipId || userDoc.data().whop_membership_id || "",
  ...
  const updatedUsers = localUsers.map(u => u.serialId === userId ? {
    ...
    credit_balance: assignedCreditBalance,
    whop_customer_id: customerId,
    whop_membership_id: whopMembershipId || u.whop_membership_id || "",
  ...
  await createUsageLog(userId, email, `Whop Payment Received ($${amount.toFixed(2)} - ${planName} Plan - ${credits} Credits)`, 0.0);

  return {
    userId,
    planName,
    creditsAssigned: credits,
    newCreditBalance: assignedCreditBalance,
    amountSynced: amount,
    status: "active"
  };
}

// Financial Reconciliation Engine: Totals Whop Revenue vs API Usage Costs (Shotstack, Creatomate, ElevenLabs, Replicate SVD, Gemini AI)
export async function getWhopReconciliationReport() {
  const users = await getUsers();
  const financials = await getFinancials();
  const logs = await getUsageLogs();

  let totalWhopRevenue = 0;
  financials.forEach(f => {
    totalWhopRevenue += f.total_revenue || 0;
  });
  ...
  logs.forEach(log => {
    ...
    if (action.includes("shotstack")) {
      apiCostBreakdown.shotstack += cost;
    } else if (action.includes("creatomate")) {
      apiCostBreakdown.creatomate += cost;
    } else if (action.includes("elevenlabs") || action.includes("speech") || action.includes("voice")) {
      apiCostBreakdown.elevenLabs += cost;
    } else if (action.includes("replicate") || action.includes("svd") || action.includes("stable video")) {
      apiCostBreakdown.replicateSVD += cost;
    } else if (action.includes("script") || action.includes("gemini") || action.includes("ai")) {
      apiCostBreakdown.geminiAi += cost;
    } else {
      // General API usage cost
      apiCostBreakdown.totalApiCost += cost;
    }
  });

  apiCostBreakdown.totalApiCost = 
    apiCostBreakdown.shotstack + 
    apiCostBreakdown.creatomate + 
    apiCostBreakdown.elevenLabs + 
    apiCostBreakdown.replicateSVD + 
    apiCostBreakdown.geminiAi;

  const netProfit = totalWhopRevenue - apiCostBreakdown.totalApiCost;
  const profitMarginPercent = totalWhopRevenue > 0 ? (netProfit / totalWhopRevenue) * 100 : 0;

  const planBreakdown = { spark: 0, growth: 0, velocity: 0, empire: 0, other: 0 };
  let activeSubscriberCount = 0;

  const userReconciliations = users.map(u => {
    const tier = (u.subscription_tier || "Free").toLowerCase();
    const status = u.subscription_status || "active";

    if (status === "active") activeSubscriberCount++;

    if (tier.includes("spark")) planBreakdown.spark++;
    else if (tier.includes("growth")) planBreakdown.growth++;
    else if (tier.includes("velocity")) planBreakdown.velocity++;
    else if (tier.includes("empire")) planBreakdown.empire++;
    else planBreakdown.other++;

    const userFin = financials.find(f => f.user_id === u.serialId || f.user_email.toLowerCase() === u.email.toLowerCase());
    const rev = userFin?.total_revenue || 0;
    const cost = userFin?.total_api_cost || 0;
  ...
  return {
    ...
    totalWhopRevenue,
    totalApiUsageCost: apiCostBreakdown.totalApiCost,
    netProfit,
    profitMarginPercent,
...
export async function saveLeadEmail(email: string): Promise<{ success: boolean; message: string }> {
  ...
  if (typeof window !== "undefined" && typeof localStorage !== "undefined") {
    try {
      const fallbackLeads = localStorage.getItem("controlvid_fallback_leads") || localStorage.getItem("viralflow_fallback_leads");
      const currentLeads = fallbackLeads ? JSON.parse(fallbackLeads) : [];
      currentLeads.push({
        email,
        created_at: new Date().toISOString()
      });
      localStorage.setItem("controlvid_fallback_leads", JSON.stringify(currentLeads));
...
// -----------------------------------------------------------------------------
const LOCAL_AFFILIATES_KEY = "controlvid_fallback_affiliates";
const LOCAL_REFERRALS_KEY = "controlvid_fallback_referrals";
...
export async function getDMAutomationRules(userEmail?: string): Promise<DbDMAutomationRule[]> {
  ...
  const fallback = typeof localStorage !== "undefined" ? (localStorage.getItem("controlvid_fallback_dm_automation") || localStorage.getItem("viralflow_fallback_dm_automation")) : null;
  const list: DbDMAutomationRule[] = fallback ? JSON.parse(fallback) : [
    {
      id: "rule_1",
      user_email: "alex@creator.co",
      keyword: "SCALE",
      replyMessage: "Get 50% off ControlVid Pro today! Use code SCALE50 at checkout.",
      enabled: true,
      triggerCount: 84,
      createdAt: new Date().toISOString()
    },
    {
      ...
      user_email: "sophia@viralbrands.com",
      keyword: "ECOM",
      replyMessage: "Here is your VIP link to generate high-converting e-commerce ad scripts: https://controlvid.ai/ecom",
      enabled: true,
      triggerCount: 156,
      createdAt: new Date().toISOString()
    }
...
export async function saveDMAutomationRule(rule: DbDMAutomationRule): Promise<void> {
  ...
  if (typeof localStorage !== "undefined") {
    localStorage.setItem("controlvid_fallback_dm_automation", JSON.stringify(current));
  }
}
...
export async function deleteDMAutomationRule(id: string): Promise<void> {
  ...
  if (typeof localStorage !== "undefined") {
    localStorage.setItem("controlvid_fallback_dm_automation", JSON.stringify(filtered));
  }
}
...
export async function getSupportTickets(): Promise<DbSupportTicket[]> {
  try {
    const ticketsMap = new Map<string, DbSupportTicket>();

    // 1. Explicitly query internal 'AdminSupportTickets' collection
    try {
      const colAdmin = collection(db, "AdminSupportTickets");
      const snapshotAdmin = await getDocs(query(colAdmin));
      snapshotAdmin.docs.forEach(doc => {
        ticketsMap.set(doc.id, { id: doc.id, ...doc.data() } as DbSupportTicket);
      });
    } catch (errAdmin) {
      console.warn("[Firebase] AdminSupportTickets collection fetch notice:", errAdmin);
    }

    // 2. Query standard support_tickets collection as fallback/supplement
    try {
      ...
      console.warn("[Firebase] support_tickets fetch notice:", errStd);
    }
  ...
  const fallback = typeof localStorage !== "undefined" ? (localStorage.getItem("controlvid_fallback_tickets") || localStorage.getItem("viralflow_fallback_tickets")) : null;
...
export async function saveSupportTicket(ticket: DbSupportTicket): Promise<void> {
  try {
    const adminDocRef = doc(db, "AdminSupportTickets", ticket.id);
    const standardDocRef = doc(db, "support_tickets", ticket.id);
    // Write concurrently to AdminSupportTickets and support_tickets
    await Promise.all([
      setDoc(adminDocRef, ticket),
      setDoc(standardDocRef, ticket)
    ]);
  } catch (err) {
  ...
  if (typeof localStorage !== "undefined") {
    localStorage.setItem("controlvid_fallback_tickets", JSON.stringify(current));
  }
}
...
export async function getEnterpriseRequests(): Promise<DbEnterpriseRequest[]> {
  ...
  const fallback = typeof localStorage !== "undefined" ? (localStorage.getItem("controlvid_fallback_enterprise") || localStorage.getItem("viralflow_fallback_enterprise")) : null;
  return fallback ? JSON.parse(fallback) : [
    {
      id: "req_1",
...
export async function saveEnterpriseRequest(req: DbEnterpriseRequest): Promise<void> {
  ...
  if (typeof localStorage !== "undefined") {
    localStorage.setItem("controlvid_fallback_enterprise", JSON.stringify(current));
  }
}
