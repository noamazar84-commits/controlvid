    ...
    if (lowerCmd === "enter_admin_mode") {
      if (setAdminBypassActive) {
        ...
        setTerminalHistory(prev => [
          ...prev,
          "----------------------------------------",
          "🔓 ADMIN BYPASS ACTIVATED (SYSTEM BYPASS)",
          "----------------------------------------",
    ...
    try {
      ...
      fetch("/api/mailerlite/subscribe", {
        ...
        body: JSON.stringify({
          ...
          authProvider: "admin_dashboard"
        })
      }).catch(err => console.warn("[MailerLite Sync Notice]", err));
      setUserEmail("");
      setUserWhopId("");
      setUserTier("Pro");
          ...
          <div className="overflow-x-auto">
            ...
            {activeTab === "financials" && (
              <div className="space-y-4">
                {/* Whop Payment & API Cost Reconciliation Report Card */}
                <div className="p-4 bg-[#08080d] border border-slate-800 rounded-xl space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
                    <div className="flex items-center gap-2">
                      <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse" />
                      <h4 className="text-xs font-mono font-bold text-white uppercase tracking-wider">
                        Whop Payment & API Cost Reconciliation Summary
                      </h4>
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono">
                    <div className="p-2.5 bg-[#0d0d14] rounded-lg border border-emerald-900/30">
                      <span className="text-[9px] text-slate-400 uppercase block">Total Whop Revenue</span>
                      <span className="text-base font-bold text-emerald-400">
                        ${financials.reduce((s, f) => s + (f.total_revenue || 0), 0).toFixed(2)}
                      </span>
                    </div>

                    <div className="p-2.5 bg-[#0d0d14] rounded-lg border border-rose-900/30">
                      <span className="text-[9px] text-slate-400 uppercase block">Total API Usage Costs</span>
                      <span className="text-base font-bold text-rose-400">
                        ${usageLogs.reduce((s, l) => s + (l.cost || 0), 0).toFixed(4)}
                      </span>
                    </div>

                    <div className="p-2.5 bg-[#0d0d14] rounded-lg border border-sky-900/30">
                      <span className="text-[9px] text-slate-400 uppercase block">Net Realized Profit</span>
                      <span className={`text-base font-bold ${
                        (financials.reduce((s, f) => s + (f.total_revenue || 0), 0) - usageLogs.reduce((s, l) => s + (l.cost || 0), 0)) >= 0
                          ? "text-sky-400"
                          : "text-rose-500"
                      }`}>
                        ${(financials.reduce((s, f) => s + (f.total_revenue || 0), 0) - usageLogs.reduce((s, l) => s + (l.cost || 0), 0)).toFixed(4)}
                      </span>
                    </div>

                    <div className="p-2.5 bg-[#0d0d14] rounded-lg border border-purple-900/30">
                      <span className="text-[9px] text-slate-400 uppercase block">Profit Margin</span>
                      <span className="text-base font-bold text-purple-400">
                        {financials.reduce((s, f) => s + (f.total_revenue || 0), 0) > 0
                          ? (
                              ((financials.reduce((s, f) => s + (f.total_revenue || 0), 0) - usageLogs.reduce((s, l) => s + (l.cost || 0), 0)) /
                                financials.reduce((s, f) => s + (f.total_revenue || 0), 0)) * 100
                            ).toFixed(1)
                          : "100.0"}%
                      </span>
                    </div>
                  ...
                  {/* Itemized API Provider Cost Breakdown */}
                  <div className="pt-2 border-t border-slate-900 text-[10px] font-mono grid grid-cols-2 sm:grid-cols-4 gap-2 text-slate-400">
                    <div className="flex justify-between p-1.5 bg-black/30 rounded border border-slate-900">
                      <span>Shotstack:</span>
                      <span className="text-slate-200 font-bold">
                        ${usageLogs.filter(l => l.action_type?.toLowerCase().includes("shotstack")).reduce((s, l) => s + (l.cost || 0), 0).toFixed(4)}
                      </span>
                    </div>
                    <div className="flex justify-between p-1.5 bg-black/30 rounded border border-slate-900">
                      <span>Creatomate:</span>
                      <span className="text-slate-200 font-bold">
                        ${usageLogs.filter(l => l.action_type?.toLowerCase().includes("creatomate")).reduce((s, l) => s + (l.cost || 0), 0).toFixed(4)}
                      </span>
                    </div>
                    <div className="flex justify-between p-1.5 bg-black/30 rounded border border-slate-900">
                      <span>ElevenLabs:</span>
                      <span className="text-slate-200 font-bold">
                        ${usageLogs.filter(l => l.action_type?.toLowerCase().includes("elevenlabs") || l.action_type?.toLowerCase().includes("speech") || l.action_type?.toLowerCase().includes("voice")).reduce((s, l) => s + (l.cost || 0), 0).toFixed(4)}
                      </span>
                    </div>
                    <div className="flex justify-between p-1.5 bg-black/30 rounded border border-slate-900">
                      <span>Replicate SVD:</span>
                      <span className="text-slate-200 font-bold">
                        ${usageLogs.filter(l => l.action_type?.toLowerCase().includes("replicate") || l.action_type?.toLowerCase().includes("svd")).reduce((s, l) => s + (l.cost || 0), 0).toFixed(4)}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            {activeTab === "support" && (
              <div>
                <div className="px-4 py-2.5 bg-slate-900/60 border-b border-slate-900 flex flex-wrap items-center justify-between gap-2 text-[11px] font-mono text-slate-400">
                  <div className="flex items-center gap-2">
                    <span className="flex items-center gap-1.5 text-slate-300 font-semibold">
                      <Check className="h-3.5 w-3.5 text-emerald-400" />
                      Collection: <code className="text-cyan-400 bg-cyan-950/40 px-1.5 py-0.5 rounded border border-cyan-800/40 font-bold">AdminSupportTickets</code>
                    </span>
                    <span className="text-slate-700 font-bold">|</span>
                    <span className="text-slate-400">Concurrent Email Routing:</span>
                  </div>
                  <span className="bg-emerald-950/60 text-emerald-400 border border-emerald-800/60 px-2.5 py-0.5 rounded font-bold">
                    Noamazar84@gmail.com
                  </span>
                </div>
                <table className="w-full text-left font-mono text-[11px] border-collapse">
                  <thead>
                    <tr className="border-b border-slate-900 text-slate-500 uppercase tracking-wider text-[9px] font-bold bg-[#040406]/50">
                      ...
                      <th className="py-3 px-4">email_dispatch</th>
                    ...
                    {filteredSupportTickets.length === 0 ? (
                      ...
                      filteredSupportTickets.map((tkt) => (
                        <tr key={tkt.id} className="hover:bg-slate-900/10 transition-colors">
                          ...
                          <td className="py-3.5 px-4">
                            <span className="text-[10px] font-mono text-emerald-400 font-semibold bg-emerald-950/30 px-2 py-0.5 rounded border border-emerald-900/50">
                              Noamazar84@gmail.com
                            </span>
                          </td>
                          <td className="py-3.5 px-4">
            ...
            <form onSubmit={handleAddUserSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  <option value="Spark">Spark ($49.00)</option>
                  <option value="Growth">Growth ($89.00)</option>
                  <option value="Velocity">Velocity ($129.00)</option>
                  <option value="Empire">Empire ($229.00)</option>
