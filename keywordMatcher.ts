...
export default function EcommerceMode({ onBack, onSuccess, activeUserEmail = "guest@controlvid.ai" }: EcommerceModeProps) {
  // State machine
  ...
  const [voiceModel, setVoiceModel] = useState<string>('Emily (SaaS Marketing)');
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        ...
        {step === 'input' && (
          <div className="magic-settings-container" style={{ 
            ...
            >
              <Sparkles size={16} />
              <span>GENERATE VIDEOS ({videoQuantity} AD {videoQuantity === 1 ? 'VIDEO' : 'VIDEOS'} • {formatCreditText("ecommerce", videoLength, videoQuantity)})</span>
            </button>
          </div>
        )}
